...
const TotalAccountsMixScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        if (_.size(creditScoreFactor) > 0) {
            const { data, video } = creditScoreFactor[3];
            setAccountsCount(data["totalAccounts"]);
            setPercent(calcPercent(data["totalAccounts"]));
            setVideoURI(video);
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer} bounces={false} showsVerticalScrollIndicator={false}>
                <View style={styles.chartWrapper}>
                    <ProgressFlatBar
                        ...
                        content={
                            <Text style={styles.accountsCountText}>{accountsCount}</Text>
                        }
                        percent={percent}
                    />
                    <View style={styles.levelsWrapper}>
                        <FactorLevelIndicator level={CreditFactorLevels["Total_account_mix"]} />
                    </View>
                </View>
...
export default TotalAccountsMixScreen;
