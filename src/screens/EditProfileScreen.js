...
const EditProfileScreen = ({ navigation }) => {
    ...
    useFocusEffect(
        useCallback(() => {
            if (userData && !name) {
                setName(userData.legal_name);
                setProfilePhotoURI(userData.image);
                setProfilePhotoB64(userData.image);
                setProfilePhotoSelected(true);
                setOccupation(userData.occupation);
                setSelectedOccupation(userData.occupation);
                setAddress1(userData.address_line_1);
                setAddress2(userData.address_line_2);
                setCity(userData.city);
                setState(userData.state);
                setZipCode(userData.zipcode);
                const dateArr = _.split(userData.dob, "/");
                setDOB(`${dateArr[1]}/${dateArr[0]}/${dateArr[2].toString().slice(2)}`);
                setDOBSend(userData.dob);
            }
    ...
    const submit = () => {
        ...
        if (profilePhotoB64 === profilePhotoURI) {
            delete user["profile_image"];
        }

        new Promise((resolve, reject) => {
            dispatch(UserActions.editProfile({ user, token: userToken, resolve, reject }));
        })
            .then(res => {
                if (res) {
                    setTimeout(() => {
            ...
            .catch(err => {
                setLoading(false);
                console.warn("Error while profile edit ---->", err)
            })
    }
    ...
    useEffect(() => {
        if (name && dob && occupation && address1 && state && city && zipCode && profilePhotoB64) {
            if (submitDisable) {
    ...
    }, [name, dob, occupation, address1, state, city, zipCode, profilePhotoB64])

    return (
        <View style={styles.container}>
            <Header title="Edit Profile" backIconPress={() => navigation.goBack()} />
            <PageView
