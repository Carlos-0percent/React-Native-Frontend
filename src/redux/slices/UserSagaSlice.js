...
const UserSagaSlice = createModule({
    ...
    reducers: {
        ...
        sendFCMTokenSuccess: (state) => {
            state.loading = false;
        },
        sendFCMTokenFailure: (state, payload) => {
            state.loading = false;
            state.error = payload;
        },
    ...
    sagas: (actions) => ({
        ...
        *[actions.sendFCMToken]({ payload }) {
            try {
                const { user, token, resolve } = payload;
                const res = yield call(UserServices.sendFCMToken, user, token);
                yield put(actions.sendFCMTokenSuccess());
                resolve(res.data);
            }
            catch (err) {
                const { reject } = payload;
                yield put(actions.sendFCMTokenFailure(err.response.data));
                reject(err.response.data);
            }
        },
        *[actions.fetchNotifications]({ payload }) {
            try {
                const { token, resolve } = payload;
                const res = yield call(UserServices.fetchNotifications, token);
                const { notificationsList } = res.data.data;
                
                const data = _.map(notificationsList, (notification) => {
                    const reqData = {};
                    reqData.id = notification.id;
                    reqData.title = notification.title;
                    reqData.description = notification.description;
                    const date = new Date(_.toNumber(notification.created_at));
                    reqData.date = `${date.getDate()} ${Month[date.getMonth()].key}`;
                    return reqData;
                })

                yield put(actions.fetchNotificationsSuccess(data));
                resolve(data);
            }
            catch (err) {
                const { reject } = payload;
                yield put(actions.fetchNotificationsFailure(err.response.data));
                reject(err.response.data);
            }
        },
    })
