...
const CreditSagaSlice = createModule({
    ...
    reducers: {
        ...
        setAccountPayHistory: (state) => {
            state.loading = true;
        },
        setAccountPayHistorySuccess: (state, payload) => {
            state.loading = false;
            state.accountPayHistory = payload.accountPayHistory;
            state.accountIndex = payload.accountIndex;
        },
        setAccountPayHistoryFailure: (state, payload) => {
            state.loading = false;
            state.error = payload;
        },
    },
    sagas: (actions) => ({
        ...
        *[actions.setAccountPayHistory]({ payload }) {
            try {
                const { accountReport, accountIndex, resolve } = payload;
                const accountReportUpdated = _.map(accountReport, (providerBasedData) => {
                    const provider = providerBasedData.provider;
                    const paymentHistory = _.map(providerBasedData.paymentHistory, (data) => {
                        const newData = _.cloneDeep(data);
                        newData.provider = provider;
                        return newData;
                    })
                    const newProviderBasedData = _.cloneDeep(providerBasedData);
                    newProviderBasedData.paymentHistory = _.map(paymentHistory);
                    return newProviderBasedData;
                })
                const payHistory = _.flatten(_.map(accountReportUpdated, "paymentHistory"));
                const data = { accountPayHistory: _.map(payHistory), accountIndex };
                yield put(actions.setAccountPayHistorySuccess(data));
                resolve();
            } catch (err) {
                const { reject } = payload;
                yield put(actions.setAccountPayHistoryFailure(err));
                reject(err);
            }
        },
    })
